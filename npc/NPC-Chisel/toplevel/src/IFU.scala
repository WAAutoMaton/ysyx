...
class IFU extends Module{
  ...
  private val state_idle :: state_read :: state_read_ready :: state_wait_ready :: Nil = Enum(4)
  val state = RegInit(state_idle)
  state := MuxLookup(state, state_idle, List(
    state_idle -> Mux(io.in.valid, state_read, state_idle),
    state_read -> state_read_ready,
    state_read_ready -> state_wait_ready,
  ...
  inst := Mux(state === state_read_ready, imem.io.rdata, inst)
